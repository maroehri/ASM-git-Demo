# Teaching Git

## First Steps

This section is supposed to contain some first steps material.

We will provide you with a list of all different git commands and their purpose.

### Git add

The command `git add` allows you to add files that have been changed to the staging area.

### Git commit

The command `git commit` is meant to store files that have been changed into the repository.

TODO: Some more explanations ...

### Git status

With the `git status` command you can get the current status of your git repo.
